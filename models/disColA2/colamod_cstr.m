function xprime=colamod_cstr(t,X,U) 
%
...
dMdt(NF) = dMdt(NF)  + F;              % add CSTR contribution
dMxdt(NF)= dMxdt(NF) + F*x(NT+1);
...
% Add CSTR model (F_0 and z_F0 are inputs!)
% k1          = 0.341/60.0; %1e8*np.exp(-6.67e4/(8.14*T_R))
k1          = 34.1/60.0; %1e8*np.exp(-6.67e4/(8.14*T_R))
dMdt(NT+1)  = F_0 + D - F;
dMxdt(NT+1) = F_0*zF + D*x(NT) - F*x(NT+1) - k1*M(NT+1)*x(NT+1);


% Compute the derivative for the mole fractions from d(Mx) = x dM + M dx
i=1:NT+1; % (N+1 because of CSTR)
