function [x_init, y_init, elapsedqp, Eta] = jpredictorLicq(problem, p_init, p_final, x_init, y_init, delta_t, lb_init, ub_init, verbose_level, N, paramModel)
...
while (t <= 1)
    ...
    if (qp_exit < 0) % QP is infeasible
        ...
        [~,g,~,~,cin,~,~,Jeq,~,~,~] = prob.obj(x_init+y, lamda, p_t, N, paramModel);
